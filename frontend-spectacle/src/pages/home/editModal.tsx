...
const EditModal: React.FC<Props> = ({ onClose, open, id }) => {

    const { data } = useSWR<Spectacle>(id ? `spectacle/${id}` : null)
    ...
    const formik = useFormik({
        ...
        initialValues: {
            name: data?.name,
            reservationPrice: String(data?.reservationPrice),
            description: data?.description,
            reservationLimit: data?.reservationLimit
        },
        onSubmit: async values => {
            await backend.put(`spectacle/${id}`, {
